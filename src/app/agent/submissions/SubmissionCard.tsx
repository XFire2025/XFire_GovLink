...
const SubmissionCard: React.FC<SubmissionCardProps> = ({
  ...
  return (
    <div className="group bg-card/90 dark:bg-card/95 backdrop-blur-md rounded-2xl border border-border/50 shadow-glow hover:shadow-2xl transition-all duration-500 hover:scale-[1.01] animate-fade-in-up modern-card">
      <div className="flex flex-col lg:flex-row lg:items-center gap-6 p-6">
        {/* Left Section - Icon & Main Info */}
        <div className="flex items-start gap-4 flex-1">
          {/* Icon Container - Same as DashboardCard */}
          <div
            className="relative flex items-center justify-center w-16 h-16 rounded-2xl transition-all duration-500 group-hover:scale-110 group-hover:rotate-6"
            style={{
              background:
          ...
          >
            <div className="text-2xl text-[#FFC72C] group-hover:text-[#FF5722] transition-all duration-300 group-hover:scale-110">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between mb-2">
              <div>
                <h3 className="text-lg font-bold text-foreground group-hover:text-[#FFC72C] transition-colors duration-300">
              ...
              {submission.priority === "urgent" && (
                <span className="inline-flex items-center px-2 py-1 bg-[#FF5722]/20 text-[#FF5722] border border-[#FF5722]/30 rounded-full text-xs font-semibold animate-pulse backdrop-blur-md">
                  {t.urgent}
                </span>
              )}
            </div>
              ...
              {submission.attachments.length > 0 && (
                <div className="flex flex-wrap gap-1 ml-2">
                  {submission.attachments.slice(0, 2).map((file) => (
                    <span
                      key={file.id}
                      className="text-xs bg-card/30 px-2 py-1 rounded text-muted-foreground"
                    >
            ...
            {submission.status === "needs_info" && submission.requestedInfo && (
              <div className="p-3 bg-[#8D153A]/10 border border-[#8D153A]/30 rounded-lg mb-4">
                <p className="text-sm font-medium text-[#8D153A] mb-1">
        ...
        <div className="flex flex-col lg:items-end gap-3 lg:min-w-[200px]">
          {/* Status Badge */}
          <span
            className={`inline-flex items-center px-3 py-2 rounded-full text-sm font-semibold border backdrop-blur-md ${getStatusColor(
              submission.status
            )}`}
          >
            ...
            {submission.status === "pending" && (
              <div className="relative">
                <button
                  ...
                  className="px-4 py-2 bg-gradient-to-r from-[#FFC72C] to-[#FF5722] text-white rounded-lg text-sm font-medium hover:from-[#FF5722] hover:to-[#8D153A] transition-all duration-300 hover:scale-105 flex items-center gap-2 shadow-lg hover:shadow-xl"
                >
                  {t.quickActions}
                  <svg
      ...
      {/* Hover Effect Gradient - EXACT SAME as DashboardCard */}
      <div className="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none">
